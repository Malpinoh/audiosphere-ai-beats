...
-- Filter regional charts to African countries only (function instead of view)
CREATE OR REPLACE FUNCTION get_african_regional_charts()
RETURNS TABLE (
    track_id uuid,
    play_count bigint,
...
BEGIN
    RETURN QUERY
    SELECT rc.track_id, rc.play_count, rc.last_played_at, rc.region_country
    FROM regional_charts rc
    WHERE rc.region_country IN (
